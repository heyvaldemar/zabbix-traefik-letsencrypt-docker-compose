#!/usr/bin/env bash
# zabbix-restore-database.sh [backup-file-name]
#
# Replaces the Zabbix database with one of the dumps the backups service wrote.
#
#   ./zabbix-restore-database.sh               list and ask
#   ./zabbix-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than zabbix.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-zabbix}"
APP_SERVICE="zabbix-server"
ALSO_STOP="zabbix-dashboard"  # it holds database connections too
...
DIR="$(env_of ZABBIX_POSTGRES_BACKUPS_PATH)"; NAME="$(env_of ZABBIX_POSTGRES_BACKUP_NAME)"
DB_NAME="$(env_of ZABBIX_DB_NAME)"; DB_USER="$(env_of ZABBIX_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Zabbix." >&2
  exit 1
fi
